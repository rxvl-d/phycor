...
class Writer:
    def __init__(self, results_dir):
        ...
        self.crop_text_dir = self.results_dir + 'crop_text/'
        if os.path.isdir(self.results_dir):
            ...
            os.makedirs(self.crop_text_dir)
        self.results = []

    def write(self, filename: str, page_index: int, el_idx: int, el_type: str,
              score: float, source_image: Image, crop: Image, orc_text: str):
        ...
        crop_text_filename = self.write_crop_text(filename, page_index, el_idx, orc_text)
        self.results.append((filename, page_index, el_idx, el_type, score,
                             page_image_filename, crop_image_filename, crop_text_filename))

    def finalize(self):
        pd.DataFrame(self.results,
                     columns=['pdf_filename', 'page_num', 'el_num', 'el_type', 'el_score',
                              'page_image', 'el_image', 'el_text']
    ...
    def write_crop_text(self, filename: str, page_index: int, el_idx: int, crop_text: str):
        crop_text_filename = filename.replace('.pdf', '') + '_' + str(page_index) + '_' + str(el_idx) + '.txt'
        dest_path = self.crop_text_dir + crop_text_filename
        if not os.path.isfile(dest_path):
