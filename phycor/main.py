...
def parse_args():
    ...
    parser.add_argument("-n", "--n_pages", help="Num. Pages", type=int)
...
def main():
    args = parse_args()
    run(args.data_dir, args.output_path, args.model_type, args.n_pages)


def run(data_dir, output_path, model_type, n_pages):
    loader = Loader(data_dir)
    parser = Parser(model_type)
    writer = Writer(output_path)
    for filename, page_index, image in islice(loader.page_images(), n_pages):
